...
class VMManagerAPI:
    ...
    def wait_for_job_completion(self, job_id: str, timeout: int = 3600) -> bool:
        ...
        try:
            # Try different possible API endpoints for VMManager Basic
            possible_urls = [
                f"{self.host}/vm/v3/task/{job_id}",  # Current endpoint
                f"{self.host}/api/v3/task/{job_id}",  # Alternative for VMManager Basic
                f"{self.host}/task/{job_id}",        # Simplified endpoint
            ]
            ...
            while time.time() - start_time < timeout:
                ...
                if status == 'success':
                    ...
                    error_msg = job_data.get('error_message', job_data.get('error', 'Unknown error'))
                    logging.error(f"Job {job_id} failed: {error_msg}")
                    return False
                elif status == 'cancelled' or status == 'canceled':
                    logging.warning(f"Job {job_id} was cancelled")
                    return False
